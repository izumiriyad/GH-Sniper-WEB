...
const DB_PATH = path_1.default.join(__dirname, '..', '..', 'data', 'ghsniper.db');
...
const addLogBatch = db.transaction((entries) => {
...
exports.DB = {
    addAccount(email, accessToken, refreshToken, expiresAt, deviceProfile, capturedHeaders) {
        const now = Date.now();
        const existing = stmts.getAccount.get(email);
        if (existing) {
        ...
        const acc = stmts.getAccount.get(email);
    ...
    getAccountById(id) {
        return stmts.getAccountById.get(id);
    },
    getAllAccounts() {
        const accounts = stmts.getAllAccounts.all();
        for (const acc of accounts)
            accountCache.set(acc.email, acc);
        return accounts;
    },
    updateTokens(email, accessToken, refreshToken, expiresAt) {
    ...
    setSniperRunning(email, running) {
        stmts.updateSniper.run(running ? 1 : 0, Date.now(), email);
        accountCache.delete(email);
    },
    setSniperInterval(email, intervalMs) {
    ...
    addLog(email, action, blockId, status, message, httpCode) {
        const acc = accountCache.get(email) || stmts.getAccount.get(email);
    ...
    addLogsBatch(entries) {
        const mapped = entries.map(e => {
            const acc = accountCache.get(e.email) || stmts.getAccount.get(e.email);
    ...
    getGrabCount(email) {
        return stmts.getGrabCount.get(email)?.count || 0;
    },
    getTotalGrabCount() {
        return stmts.getTotalGrabCount.get()?.count || 0;
    },
    performMaintenance() {
    ...
    getStats() {
        ...
        const logs = db.prepare('SELECT COUNT(*) as count FROM logs').get()?.count || 0;
        const grabs = db.prepare('SELECT COUNT(*) as count FROM grabs').get()?.count || 0;
        const walSize = (() => { try {
            return fs_1.default.statSync(DB_PATH + '-wal').size;
        }
        catch {
            return 0;
        } })();
        const dbSize = (() => { try {
            return fs_1.default.statSync(DB_PATH).size;
        }
        catch {
            return 0;
        } })();
