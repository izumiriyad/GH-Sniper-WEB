...
import { continuousPickup, refreshToken, isTokenExpiring, SniperResult } from '../api/grubhubApi';
...
interface SniperInstance {
  ...
  warmupTimer: ReturnType<typeof setInterval> | null; // Track interval to prevent leak
}
...
export function getSniperStatus(email: string): SniperInstance | undefined {
  return snipers.get(email);
}

export function getAllSniperStatuses(): SniperInstance[] {
  return [...snipers.values()];
}

export async function startSniper(email: string, intervalMs = 3000): Promise<{ started: boolean; message: string }> {
  ...
  DB.setSniperRunning(email, true);

  // Ensure global connection warming is active
  startConnectionWarming();

  DB.addLog(email, 'SNIPER_START', undefined, 'ok', 'Interval: ' + intervalMs + 'ms');
  ...
  (async () => {
    while (!instance.stopFlag) {
      try {
        ...
        if (instance.cycles % 100 === 0) triggerGC();
        const result = await continuousPickup(
          email,
          intervalMs,
          (msg) => {
            ...
            if (m) instance.cycles = parseInt(m[1]);
        ...
        if (!instance.stopFlag) {
          ...
          DB.addLog(email, 'SNIPER_RESTART', undefined, 'warn', 'Auto-restarting: ' + result.message);
        ...
        DB.addLog(email, 'SNIPER_ERROR', undefined, 'error', e.message);
        await new Promise(r => setTimeout(r, 5000));
      }
    }
    instance.running = false;
    DB.setSniperRunning(email, false);
  })();

  return { started: true, message: 'Sniper started for ' + email + ' (' + intervalMs + 'ms)' };
}

export function stopSniper(email: string): { stopped: boolean; message: string } {
  ...
  inst.stopFlag = true;
  DB.setSniperRunning(email, false);
  DB.addLog(email, 'SNIPER_STOP', undefined, 'ok', 'Stopped after ' + inst.cycles + ' cycles');
...
setInterval(async () => {
  ...
  for (const acc of accounts) {
    if (acc.status === 'active' && acc.access_token && isTokenExpiring(acc.email)) {
      console.log('[TokenRefresh] Refreshing ' + acc.email);
      await refreshToken(acc.email);
    }
  }
}, 30 * 60 * 1000);
...
export function autoRestartSnipers() {
  const accounts = DB.getAllAccounts();
  for (const acc of accounts) {
